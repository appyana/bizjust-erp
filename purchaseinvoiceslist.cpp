...
#include "ui_purchaseinvoiceslist.h"

PurchaseInvoicesList::PurchaseInvoicesList(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::PurchaseInvoicesList)
{
    ui->setupUi(this);
...
void PurchaseInvoicesList::autocompleter(QString sql, QLineEdit *name_txt, QLineEdit *id_txt)
{
...
void PurchaseInvoicesList::onItemHighlighted(const QModelIndex &index)
{
...
void PurchaseInvoicesList::editingFinished()
{
...
void PurchaseInvoicesList::loadParams()
{
    timer->stop();
...
void PurchaseInvoicesList::settableHeaders()
{
    QStringList titles;
    ui->tableWidgetList->setColumnCount(4);
    titles <<"Invoice #"<<"Vendor"<<"Total"<<"Date";
...
void PurchaseInvoicesList::on_vendorname_textEdited(const QString &arg1)
{
    autocompleter(sch.vendors(arg1), ui->vendorname, ui->vendorid);
}

void PurchaseInvoicesList::on_btn_search_clicked()
{
    ui->tableWidgetList->setRowCount(0);
    QString sql = "SELECT * from tblinvporcv where rcvid !='' and rcvdate BETWEEN '"+ui->begindate->date().toString("yyyy-MM-dd")+"' AND '"+ui->enddate->date().toString("yyyy-MM-dd")+"' ";
    if(ui->invoiceNo->text()!="")
    {
        sql += " AND (voucherno = '"+ui->invoiceNo->text()+"' or voucherno = 'pi-"+ui->invoiceNo->text()+"') ";
    }
    if(ui->vendorid->text()!="")
    {
        sql += " and vendorid = '"+ui->vendorid->text()+"' ";
    }
    sql += " order by rcvdate desc ";
    QSqlQuery qry;
    qry.prepare(sql);
    if(qry.exec())
    {
        ...
        while(qry.next())
        {
            ...
            ui->tableWidgetList->setItem(i,INVOICENO, new QTableWidgetItem(qry.value("voucherno").toString()));
            ui->tableWidgetList->setItem(i,VENDOR, new QTableWidgetItem( erp.vendorname(qry.value("vendorid").toString()) ));
            ui->tableWidgetList->setItem(i,DATE, new QTableWidgetItem( erp.DDMMYYDateFromSQL( qry.value("rcvdate").toString()) ) );
            ui->tableWidgetList->setItem(i,TOTAL, new QTableWidgetItem( erp.DecimalString( qry.value("total").toFloat() )) );
            i++;
        }
    }
...
void PurchaseInvoicesList::on_btn_printInvoice_clicked()
{
...
void PurchaseInvoicesList::loadInvoicePrint()
{
    ...
    pd->setWindowTitle("Print Purchase Invoice");
...
void PurchaseInvoicesList::printPreview(QPrinter *p)
{
    QString vno = ui->tableWidgetList->item(ui->tableWidgetList->currentRow(),INVOICENO )->text();

    QString sql = " select * from tblinvporcv where voucherno = '"+vno+"' ";
    QSqlQuery qry;
    qry.prepare(sql);
    qry.exec();
    qry.next();
...
/**************************************** Detail *******************************************************/
    ...
    html += "<table width='100%'>"
            ...
            "<table width='100%' style='font-size:12px;'>"
            "<tr>"
            "<td><!--<b>Invoice No:</b> "+qry.value("voucherno").toString()+"<br>--><b>Invoice Date:</b> "+qry.value("rcvdate").toDate().toString("dd/MM/yyyy")+"</td>"
    ...
    QString sqldet = "SELECT * FROM tblinvporcvdetail where rcvid = '"+qry.value("rcvid").toString()+"' order by rcvdetid";
    QSqlQuery det;
    det.prepare(sqldet);
    int ln=1;
    float total=0.00;
    if(det.exec())
    {
        while(det.next())
        {
            html += "<tr>"
                    ...
                    "<td align='center'>"+erp.intString(det.value("qtyrcv").toInt())+"</td>"
...
/******************************************* Header****************************************************/
    ...
    QString header_html = "<table width='100%'>"
                          ...
                          "</tr>"
                          "<tr>"
                          "<th style='font-size:16px;'>Purchase Invoice</th>"
                          "</tr>"
