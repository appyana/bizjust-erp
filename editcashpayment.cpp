...
#include "ui_editcashpayment.h"

EditCashPayment::EditCashPayment(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::EditCashPayment)
{
    ui->setupUi(this);
    loadform();
}
...
void EditCashPayment::autocompleter(QString sql, QLineEdit *name_txt, QLineEdit *id_txt)
{
...
void EditCashPayment::onItemHighlighted(const QModelIndex &index)
{
...
void EditCashPayment::editingFinished()
{
...
void EditCashPayment::on_accountname_textEdited(const QString &arg1)
{
   QString sql = sch.glaccount_generalize(arg1);
   autocompleter(sql,ui->accountname, ui->accountid);
}

void EditCashPayment::loadform()
{
...
void EditCashPayment::on_btn_add_clicked()
{
...
void EditCashPayment::checktotal()
{
    float tot_amt=0;
...
void EditCashPayment::on_btn_delete_row_clicked()
{
...
void EditCashPayment::on_btn_save_clicked()
{
...
void EditCashPayment::on_btn_search_clicked()
{
    ...
    QString sql = " select * from tblgltransvoucher where voucherno like '%cpv-%' and entrydate BETWEEN '"+begindate+"' AND '"+enddate+"' ";
    if(vno!="")
    {
        sql += " and (voucherno = '"+vno+"' or voucherno = 'cpv-"+vno+"') ";
    }
...
void EditCashPayment::on_btn_edit_clicked()
{
    if(ui->searchTable->currentRow()>=0)
    {
        ...
        QSqlQuery info(" select * from tblgltransaction where voucherno = '"+vno+"' and debit > '0.00' order by linenumber ");
        while(info.next())
        {
            int row = ui->detailtable->rowCount();
            ui->detailtable->insertRow(row);
            ...
            ui->detailtable->setItem(row,AMOUNT, new QTableWidgetItem( erp.DecimalString( info.value("debit").toFloat() ) ));
            checktotal();
        }
...
void EditCashPayment::on_btn_delete_clicked()
{
