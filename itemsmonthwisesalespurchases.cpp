...
#include "ui_itemsmonthwisesalespurchases.h"

ItemsMonthWiseSalesPurchases::ItemsMonthWiseSalesPurchases(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::ItemsMonthWiseSalesPurchases)
{
    ui->setupUi(this);
    loarForm();
}

ItemsMonthWiseSalesPurchases::~ItemsMonthWiseSalesPurchases()
{
    delete ui;
}


void ItemsMonthWiseSalesPurchases::autocompleter(QString sql, QLineEdit *name_txt, QLineEdit *id_txt)
{
...
void ItemsMonthWiseSalesPurchases::onItemHighlighted(const QModelIndex &index)
{
...
void ItemsMonthWiseSalesPurchases::editingFinished()
{
...
void ItemsMonthWiseSalesPurchases::loarForm()
{
    ui->detailFrame->setVisible(false);
    QFont font("Segoe UI", 9, QFont::Bold);
    QStringList titles;
    titles <<"Month"<<"Qty Sold"<<"Sales"<<"Qty Purchased"<<"Purchases";
    ...
    ui->detailTable->horizontalHeader()->setSectionResizeMode(3, QHeaderView::Stretch);
    ui->detailTable->horizontalHeader()->setSectionResizeMode(4, QHeaderView::Stretch);
    for(int i=0; i<13; i++)
    {
        ...
        ui->detailTable->setItem(i,3,new QTableWidgetItem(""));
        ui->detailTable->setItem(i,4,new QTableWidgetItem(""));
        ...
        ui->detailTable->item(i,3)->setTextAlignment(Qt::AlignCenter);
        ui->detailTable->item(i,4)->setTextAlignment(Qt::AlignCenter);
    }
    ...
    ui->detailTable->item(12,3)->setTextAlignment(Qt::AlignCenter);
    ui->detailTable->item(12,4)->setTextAlignment(Qt::AlignCenter);
    ui->detailTable->item(12,1)->setFont(font);
    ui->detailTable->item(12,2)->setFont(font);
    ui->detailTable->item(12,3)->setFont(font);
    ui->detailTable->item(12,4)->setFont(font);
}

void ItemsMonthWiseSalesPurchases::viewDetail()
{
    ...
    for(int i=1; i<=12; i++)
    {
        ...
        QString piqtysold = erp.saleAndPurchasedUnits("pi",itemid,foryear,month,true);
        float purchases = erp.saleAndPurchasePrice("pi",itemid,foryear,month);
        //qDebug()<<piqtysold;
        totpiqty += erp.saleAndPurchasedUnits("pi",itemid,foryear,month,false).toInt();
        totpiprice += purchases;



        ui->detailTable->item(row,1)->setText(qtysold);
        ui->detailTable->item(row,2)->setText( erp.amountString(sales) );
        ui->detailTable->item(row,3)->setText(piqtysold);
        ui->detailTable->item(row,4)->setText( erp.amountString(purchases) );
    }
    ui->detailTable->item(12,1)->setText( erp.intString(netunit) );
    ui->detailTable->item(12,2)->setText( erp.amountString(price) );
    ui->detailTable->item(12,3)->setText( erp.intString(totpiqty) );
    ui->detailTable->item(12,4)->setText( erp.amountString(totpiprice) );
}

void ItemsMonthWiseSalesPurchases::on_itemname_textEdited(const QString &arg1)
{
    autocompleter(sch.items(arg1),ui->itemname,ui->itemid);
}

void ItemsMonthWiseSalesPurchases::on_btn_next_clicked()
{
    if(ui->itemid->text()=="")
    {
...
void ItemsMonthWiseSalesPurchases::on_previous_year_btn_clicked()
{
    foryear = erp.intString(foryear.toInt()-1);
    viewDetail();
}

void ItemsMonthWiseSalesPurchases::on_next_year_btn_clicked()
{
    foryear = erp.intString(foryear.toInt()+1);
    viewDetail();
}
