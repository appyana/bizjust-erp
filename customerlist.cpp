...
void CustomerList::loadCustomers()
{

    //QSqlQueryModel *model = new QSqlQueryModel();
    QStandardItemModel *model=new QStandardItemModel();
    QSqlQuery qry;
    QString sql = "select name,address1, city, CONCAT(entry_firstname,' ', middle_name,' ',entry_lastname) as fullname, phone1, email1, groupid, discount  from tblcustomers where dl =0 ";
    qry.prepare(sql);
    qry.exec();
    int row=0;
