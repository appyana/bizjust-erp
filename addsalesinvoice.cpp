...
#include "ui_addsalesinvoice.h"

AddSalesInvoice::AddSalesInvoice(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::AddSalesInvoice)
{
    ui->setupUi(this);
...
//    ui->txt_qty->setValidator( new QRegExpValidator(QRegExp("[0-9.-.]*")) );
//    ui->txt_price->setValidator(new QRegExpValidator(QRegExp("[0-9.-.]*")));//allow only numbers and decimals/floats/doubles
//    ui->txt_amt->setValidator(new QRegExpValidator(QRegExp("[0-9.-.]*")));
//    ui->txt_totalamt->setValidator(new QRegExpValidator(QRegExp("[0-9.-.]*")));
//    ui->txt_totNetAmount->setValidator(new QRegExpValidator(QRegExp("[0-9.-.]*")));
//    ui->txt_discount->setValidator(new QRegExpValidator(QRegExp("[0-9.-.]*")));
//    ui->txt_freight->setValidator(new QRegExpValidator(QRegExp("[0-9.-.]*")));
}
...
void AddSalesInvoice::autocompleter(QString sql, QLineEdit *name_txt, QLineEdit *id_txt)
{
...
void AddSalesInvoice::onItemHighlighted(const QModelIndex &index)
{
...
void AddSalesInvoice::editingFinished()
{
...
void AddSalesInvoice::on_txt_qty_valueChanged()
{
    checkamt();
}

void AddSalesInvoice::on_txt_price_valueChanged()
{
    checkamt();
}

void AddSalesInvoice::on_txt_amt_valueChanged()
{
    checktotal();
}

void AddSalesInvoice::checkamt()
{
...
void AddSalesInvoice::on_btn_add_clicked()
{
...
void AddSalesInvoice::on_btn1_clicked()
{
    ui->tableWidgetDetail->removeRow(ui->tableWidgetDetail->currentRow());
    checktotal();
}
void AddSalesInvoice::checktotal()
{
    double total=0.00;
...
void AddSalesInvoice::on_txt_discount_valueChanged()
{
    checktotal();
}

void AddSalesInvoice::on_txt_freight_valueChanged()
{
    checktotal();
}

void AddSalesInvoice::on_txt_customerid_textChanged()
{
    checktotal();
}

void AddSalesInvoice::on_dateEntrydate_userDateChanged()
{
    checktotal();
}

void AddSalesInvoice::on_txt_totalamt_valueChanged()
{
    checktotal();
}

void AddSalesInvoice::on_txt_totNetAmount_valueChanged()
{
    checktotal();
}

void AddSalesInvoice::on_comboSales_type_currentIndexChanged()//int index
{
    checktotal();
}

void AddSalesInvoice::on_btn_save_clicked()
{
    if(isValid())
    {
        ...
        {
            QString query = "insert into tblinvoice "
                     "(invoicenumber, customerid, invoicedate, notes,  totalDiscount, invoicetotal, `lines`, `customerbillcode`, `vendorRefno`,`shipped_via`,`freight`, `sales_type`,`Carrier_id`,`AdvancePayment`,`net_total` , userid , `shipto`  )"
                     " values "
                    "('"+ord_num+"', '"+customerid+"', '"+invdate+"', '"+notes+"', '"+erp.DecimalString(totalDiscount)+"', '"+erp.DecimalString(net_total)+"', '"+erp.intString(lines)+"', '"+refno+"', '"+vendor_refno+"', '"+shipped_via+"', '"+erp.DecimalString(freight)+"', '"+sales_type+"','"+Carrier_id+"','"+erp.DecimalString(cashreceived)+"','"+erp.DecimalString(net_total)+"' , '"+loginid+"' , '"+shipto+"') ";
            QSqlQuery qry;
            qry.prepare(query);
            if(!qry.exec())
            {
                ...
                if(erp.update_ids_num(ord_code))
                {
                    ...
                    QString query_transaction = "INSERT INTO `tblgltransvoucher` ( "
                            ...
                            " VALUES "
                            " ('"+vno+"', '"+ordernum+"' , '"+notes + shipto+"', '"+invdate+"', 'Sales Invoice', '"+customerid+"', '"+erp.DecimalString(net_total)+"', '"+erp.DecimalString(net_total)+"','2','customer')";
                    ...
                    QString query_itemtrans = "insert into tblitemtrans "
                            ...
                            " values "
                            " ('"+si_num+"', 'si', '"+invdate+"', '"+customerid+"', '"+TransDesc+"', '"+ordernum+"', '"+invdate+"', '1')" ;
                    ...
                    QString msg = "Sales Invoice "+ord_num+" added Successfully!";
...
void AddSalesInvoice::clearform()
{
...
void AddSalesInvoice::on_txt_itemid_textChanged(const QString &arg1)
{
